...
class Adafruit_LSM9DS1
{
  public:
    ...
    //esp_err_t spixfer      (boolean type, uint8_t data, byte len, uint8_t *buffer);
    esp_err_t digitalRead (boolean type, uint8_t reg, byte len, uint8_t *buffer);
    esp_err_t digitalWrite (boolean type, uint8_t reg, byte value);
    ...
    public:
      ...
      virtual void getEvent(sensors_event_t* event) {
        ...
        (_parent->*_readFunc)();
        /* Fill in event data. */
        (_parent->*_eventFunc)(event, static_cast<uint32_t>(xTaskGetTickCount()));
      }
  ...
  private:
    //boolean _i2c;
    //TwoWire* _wire;
    spi_device_handle_t xgDevice;
    spi_device_handle_t mDevice;

    gpio_num_t  _csm, _csxg, _mosi, _miso, _clk;
    float   _accel_mg_lsb;
